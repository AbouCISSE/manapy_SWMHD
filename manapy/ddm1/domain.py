...
from numpy import asarray, int64, double, zeros, where, unique, sort, ones, array, concatenate, sqrt
import meshio

from manapy.ddm1.pyccel_ddm import variables, face_gradient_info_2d, variables_3d
from manapy.ddm1.numba_ddm import face_gradient_info_3d, read_mesh_file

from manapy.comms1 import update_haloghost_info_2d, update_haloghost_info_3d, prepare_comm
from mpi4py import MPI

from manapy.ddm1.pyccel_ddm import (create_info_2dfaces, create_info_3dfaces,
...
from manapy.ddm1.numba_ddm import (create_2doppNodeOfFaces, create_3doppNodeOfFaces, update_pediodic_info_2d,
...
class Domain():
    ...
    def save_on_cell(self, dt=0, time=0, niter=0, miter=0, value=None,valuex=None):
        ...
        data = {"w" : value, "wex": valuex}
        data = {"w" : data , "wex": data}
        ...
        if(self.comm.rank == 0 and self.comm.size > 1):
            with open("results/visu"+str(miter)+".pvtu", "a") as text_file:
                ...
                text_file.write("<PDataArray type=\"Float64\" Name=\"wex\" format=\"binary\"/>\n")
    ...
    def save_on_node(self, dt=0, time=0, niter=0, miter=0, valueh=None, valuehu=None, valuehv=None,
                     valuehB1=None, valuehB2=None, valueZ=None):
        
        if valueh is None:
            raise ValueError("value must be given")
        assert len(valueh) == self.nbnodes, 'valueh size != number of nodes'
        
        elements = {self.typeOfCells: self.cells._nodeid}
        points = self._nodes._vertex[:, :3]
        
#        data = {"h" : valueh, "u" : valuehu/valueh, "v" : valuehv/valueh, "B1" : valuehB1/valueh, "B2" : valuehB2/valueh, "h+b" : valueh+valueZ, "b" : valueZ, 'vol' : self.cells._volume, "B": sqrt((valuehB1/valueh)**2 + (valuehB2/valueh)**2), "U": sqrt((valuehu/valueh)**2 + (valuehv/valueh)**2) }
        
        data = {"h" : valueh, "u" : valuehu/valueh, "v" : valuehv/valueh, "B1" : valuehB1/valueh, "B2" : valuehB2/valueh, "h+b" : valueh+valueZ, "b" : valueZ, 'vol' : self.cells._volume, "B": sqrt((valuehB1/valueh)**2 + (valuehB2/valueh)**2), "U": sqrt((valuehu/valueh)**2 + (valuehv/valueh)**2) }
        
        maxw = max(valueh+valueZ)
        minw = min(valueh+valueZ)
        
        integral_maxw = zeros(1)
        integral_minw = zeros(1)
    
        self.comm.Reduce(maxw, integral_maxw, MPI.MAX, 0)
        self.comm.Reduce(minw, integral_minw, MPI.MIN, 0)
        ...
        if(self.comm.rank == 0 and self.comm.size > 1):
            with open("results/visu"+str(miter)+".pvtu", "a") as text_file:
                ...
                text_file.write("<PDataArray type=\"Float64\" Name=\"vol\" format=\"binary\"/>\n")
