...
__all__ = ["generate_mesh", "prepare_comm", "define_halosend", "all_to_all_struct", "all_to_all"]
...
class Cells:
    ...
    def __init__(self, nodeid, faceid, center, volume, globalindex, cellfid, cellnid, halonid, nf):
...
#    clear_class(Cells, Nodes, Faces)
    ...
    for i in range(len(Cells.nodeid)):
        ...
        Cells.volume.append((1./2) * abs((x_2-x_1)*(y_3-y_1)-(y_2-y_1)*(x_3-x_1)))
    ...
    for i in range(len(faces)):
        ...
        normal, mesure = normalvector(np.asarray(Nodes.vertex[faces[i][0]]),
                                         ...
                                         np.asarray(Cells.center[Faces.cellid[i][0]]))
...
def create_halo_structure(file):
    ...
    for line in file:
        ...
        if "globalcelltolocal" in line:
            break
        Halo.centvol.append([np.double(x) for x in line.split()])
    ...
    if SIZE == 1 : 
        Nodes.halonid = np.zeros((len(Nodes.name),2), dtype=int)
        Halo.centvol  = np.zeros((2,2))
        Halo.halosint = np.zeros((2,2))
    ...
    if SIZE == 1 :
        Cells.halonid = np.zeros((len(Cells.nodeid),2), dtype=int)

    
    cells = Cells(np.asarray(Cells.nodeid), np.asarray(Cells.faceid), np.asarray(Cells.center),
                  np.asarray(Cells.volume), Cells.globalindex, np.asarray(Cells.cellfid), 
                  np.asarray(Cells.cellnid),  np.asarray(Cells.halonid), np.asarray(Cells.nf))
...
def all_to_all_struct(w_halosend, taille, mystruct, variables, scount, sdepl, rcount, rdepl, w_halorecv):
...
def all_to_all(w_halosend, taille, scount, sdepl, rcount, rdepl, w_halorecv):
