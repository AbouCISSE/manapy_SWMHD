...
from manapy.comms1 import all_to_all
from manapy.comms1.pyccel_comm import define_halosend
...
class Variable():
    ...
    def __init__(self, domain=None, terms=None, comm=None, name=None, BC=None, values=None, *args, **kwargs):
        ...
        if self.dim == 2:
            ...
            self._face_gradientuv  = face_gradient_2d_uv
    ...
    def compute_face_gradient_uv(self, h, h_ghost, h_halo,  h_node):
        

        self.interpolate_celltonode()
        self._face_gradientuv(h, h_ghost, h_halo,  h_node,
                            self.cell, self.ghost, self.halo, self.node, self.domain.faces.cellid, 
                            self.domain.faces.nodeid, self.domain.faces.ghostcenter, self.domain.faces.name, 
                            self.domain.faces.halofid, self.domain.cells.center,
                            self.domain.halos.centvol, self.domain.nodes.vertex, self.domain.faces.airDiamond,
                            self.domain.faces.normal, self.domain.faces.f_1, self.domain.faces.f_2, 
    ...
    def dissipation_rate(self, Eint):
        
        assert self.nbcells == len(Eint), 'Eint solution must have length of cells'        
        ratediss = np.zeros(self.nbcells)       
        for i in range(len(Eint)):
            ratediss[i] = self.cell[i]/Eint[i]
