...
if dim == 2:
    ...
    filename = os.path.join(MESH_DIR, "EC2.msh")     
...
if RANK == 0:
    #Read gmsh file
    readmesh(filename, dim=dim, size=SIZE, periodic=[1,1])
...
tfinal = 1
iterfinal = 9000000
SCHEME =  "SRNH-GLM"  # "LF" or "GLM" or "SRNH-GLM" or "SRNH"
order = 1
saving_at_node = 1
...
ddm.plot_tricontour( w_node["h"], nodes.cellid, cells.center, nodes.vertex, 1)
ddm.plot_tricontour( np.sqrt((w_node["hu"]/w_node["h"])**2 + (w_node["hv"]/w_node["h"])**2), nodes.cellid, cells.center, nodes.vertex,2)
ddm.plot_tricontour( np.sqrt((w_node["hB1"]/w_node["h"])**2 + (w_node["hB2"]/w_node["h"])**2), nodes.cellid, cells.center, nodes.vertex,3)
